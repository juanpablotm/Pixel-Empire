...
function releaseProject(state: GameState, project: Project): GameState {
  const { q, breakdown } = computeQuality(project, {
    era: state.era,
    teamFactor: balance.quality.teamFactorGaraje,
  ...
  const released: ReleasedGame = {
    ...
    breakdown,
    lines: buildReviewLines(breakdown, project),
 ...
 * Acumula puntos de Diseño/Técnica según el reparto de esfuerzo, deuda de bugs
 * en Concepto/Producción, inversión de QA en Pulido, y avanza de fase o lanza.
 */
export function advanceProjects(state: GameState): GameState {
  ...
  let worked: Project = {
    ...project,
    weeksSpent,
    designPoints: project.designPoints + design,
    techPoints: project.techPoints + tech,
    qaInvested: project.qaInvested + qa * balance.development.qaReductionPerWeek,
    bugDebt: project.bugDebt + (project.phase < 3 ? balance.development.baseBugsPerWeek : 0),
  };
